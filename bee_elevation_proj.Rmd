...
elevation_all_ecoregions_plot <- ggplot() +
  geom_raster(data = elevation_crop_df, aes(x = x, y = y, fill = OR_DEM_10M_files)) +
...
st_crs(ecoregions)
ecoregion_list <- split(ecoregions, ecoregions$Ecoregion)

elevation_by_ecoregion <- list()

for(i in names(ecoregion_list)){
  
  ecoregion <- ecoregion_list[[i]]
  
  masked_rast <- mask(dem_rast_downsampled, ecoregion)
  
  cropped_rast <- crop(masked_rast, ecoregion)
  
  elevation_by_ecoregion[[i]] <- cropped_rast
}
print(elevation_by_ecoregion)

elevation_values <- list()


for (i in names(elevation_by_ecoregion)) {

  elevation_raster <- elevation_by_ecoregion[[i]]
  
  elevation_values[[i]] <- data.frame(elevation = elevation_values[[i]])
}

head(elevation_values[["East Cascades"]])
```
